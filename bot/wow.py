...
async def get_data(auth_token, name, field = ""):
    ...
    try:
        async with aiohttp.ClientSession() as client:
            if field == "":
                ...
                api_path = ( "%s/profile/wow/character/silvermoon/%s/%s?namespace=profile-eu&locale=en_GB&access_token=%s" 
                        % (base_api_path, name, field, auth_token) )
...
async def get_character_info(name):
    ...
    thumbnail = await get_data(auth_token, name, "character-media")
